/**
 * Builds `src/engine/airport-data.generated.ts` from OpenFlights airports.dat.
 *
 * Filter rules:
 ...
 *
 * Output must stay a pure function of the vendored input — no clock, no
 * environment, no network — because `vendor.test.ts` regenerates and compares.
 */
...
export const VENDOR_FILE = join(import.meta.dirname, '..', '..', 'vendor', 'openflights-airports.dat')
export const OUTPUT_FILE = join(import.meta.dirname, '..', '..', 'src', 'engine', 'airport-data.generated.ts')
...
function readAirports(vendorFile: string): AirportRow[] {
...
function buildAirportFromRow(
  ...
  if (!countryCode) {
    ...
    skipped.push(`${row.iata}: cannot resolve countryCode for "${row.country}"`)
    return null
  }
...
export interface GenerationResult {
  ...
  droppedDupe: number
  /** Rows dropped because no country code could be resolved — extend COUNTRY_OVERRIDES. */
  skipped: string[]
}

/** Returns the exact text of `airport-data.generated.ts` for the vendored input. */
export function generateAirportData(vendorFile: string = VENDOR_FILE): GenerationResult {
  ...
  const rows = readAirports(vendorFile)
  ...
  for (const row of rows) {
    const a = buildAirportFromRow(row, cities, cityAliasSet, citySlugSet, countryToCode, skipped)
  ...
  return { text: emitFile(airports), loaded: rows.length, kept: airports.length, droppedNoMatch, droppedDupe, skipped }
}
